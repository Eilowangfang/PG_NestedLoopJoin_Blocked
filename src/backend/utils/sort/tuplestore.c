...
{
    elog(INFO, "     ------>------>------> Fang tuplestore_tuple_count %ld %ld %ld %ld",
         state->activeptr,  state->readptrcount, state->readptrs[0].current, state->memtupcount);
	return state->tuples;
}
...
//    if(mycounter % 100 == 0) {
//        elog(INFO, "     ------>------>------> Fang ExecMaterial outerslot %lf ms\n", (mytimer / CLOCKS_PER_SEC) * 1000);
//    }
}
