...
//Fang
double      current_node_estimation = 0.0;
bool        skip_sort_cost = false;
			 ...
			 RelOptInfo *baserel, ParamPathInfo *param_info)
{
    ...
    if(!lpce_check) {
        for(int i = 0; i < NODE_NUM_MAX; i++){
            int inner_node_id = card_runLog[i].inner_node_id;
            int outer_node_id = card_runLog[i].outer_node_id;

            if(card_runLog[inner_node_id].done_work && card_runLog[i].inner_relids == baserel->relids->words[0]){
                startup_cost = 0;
                disk_run_cost = 0;
            }

            if(card_runLog[outer_node_id].done_work && card_runLog[i].outer_relids == baserel->relids->words[0]){
                startup_cost = 0;
                disk_run_cost = 0;
            }
        }
		   ...
		   bool partial_path)
{
    ...
    //elog(INFO, "     ------>------>------> Fang final_cost_index %.0f cost %lf %lf", path->path.rows, startup_cost, startup_cost + run_cost);
...

{
	...
	if(skip_sort_cost){
        startup_cost = 0;
        run_cost = 0;
        elog(INFO, "     Fang skip_sort_cost ############################## %lf", tuples);
	}


//    if(!lpce_check){
//        for(int i = 0; i < NODE_NUM_MAX; i++){
//            //the hash node has appeared in initial execution
//            if(node_style[i] == 6 && abs(tuples - card_runLog[i].estimate_card) < 1.0){
//                // if the outer path of sort node is done work
//                // we skip count the cost
//                int outer_sort_node_id = card_runLog[i].outer_node_id;
//                if(card_runLog[outer_sort_node_id].done_work){
//                    startup_cost = 0.0;
//                    run_cost = 0.0;
//                }
//            }
					  ...
					  JoinPathExtraData *extra)
{
     ...
     * The re-opt point is at outer path of nested loop path.
     * During the re-optimization, the cost of outer path can be regareded
     * as 0
     * */
    if(!lpce_check){
        for(int i = 0; i < NODE_NUM_MAX; i++){
            //elog(INFO, "     ------>------>------> Fang node id %ld  and  %ld  and      %ld  %ld", i, card_runLog[i].done_work, card_runLog[i].outer_relids, card_runLog[i].inner_relids);

            //we dont consider the done work of hash node from hash join node as done work.
            //as now the intermediate the results of hash node is a hash table,
            //which cannot be directly utilized by nested loop join.
            if(node_style[i] == 7)
                continue;
            ...
            if(card_runLog[i].inner_relids == inner_path->parent->relids->words[0]){
                if(card_runLog[inner_node_id].done_work){
                    inner_path->startup_cost = 0;
                    inner_path->total_cost = 0;
                }
            }

            if(card_runLog[i].inner_relids == outer_path->parent->relids->words[0]){
                if(card_runLog[inner_node_id].done_work){
                    outer_path->startup_cost = 0;
                    outer_path->total_cost = 0;
                }
            }

            if(card_runLog[i].outer_relids == inner_path->parent->relids->words[0]){
                if(card_runLog[outer_node_id].done_work){
                    inner_path->startup_cost = 0;
                    inner_path->total_cost = 0;
                }
            }

            if(card_runLog[i].outer_relids == outer_path->parent->relids->words[0]){
                if(card_runLog[outer_node_id].done_work){
                    outer_path->startup_cost = 0;
                    outer_path->total_cost = 0;
                }
					   ...
					   JoinPathExtraData *extra)
{
    ...
    //elog(INFO, "     Fang initial_cost_mergejoin inner ############################## %ld", inner_path->parent->relids->words[0]);
    //elog(INFO, "     Fang initial_cost_mergejoin outer ############################## %ld", outer_path->parent->relids->words[0]);

	//Fang
    bool skip_inner_sort_reopt = false;
    bool skip_outer_sort_reopt = false;

    if(!lpce_check){
        for(int i = 0; i < NODE_NUM_MAX; i++){
            //we dont consider the done work of hash node from hash join node as done work.
            //as now the intermediate the results of hash node is a hash table,
            //which cannot be directly utilized by merge join.
            if(node_style[i] == 7)
                continue;

            //the outer sort node is done work
            int outer_node_id = card_runLog[i].outer_node_id;
            int inner_node_id = card_runLog[i].inner_node_id;

            if(card_runLog[i].inner_relids == inner_path->parent->relids->words[0]){
                if(card_runLog[inner_node_id].done_work){
                    skip_inner_sort_reopt = true;
                }
            }

            if(card_runLog[i].outer_relids == outer_path->parent->relids->words[0]){
                if(card_runLog[outer_node_id].done_work){
                    skip_outer_sort_reopt = true;
                }
            }
        }
    }
    ...
    //Fang
    skip_sort_cost = false;
    if(skip_inner_sort_reopt) {
        skip_sort_cost           = true;
        inner_path->startup_cost = 0;
        inner_path->total_cost   = 0;
    }
					 ...
					 JoinPathExtraData *extra)
{
    ...
    //elog(INFO, "     ------>------>------> Fang final_cost_mergejoin  %lf and %lf and %lf ", mergejointuples, inner_rows, outer_rows);
					...
					JoinPathExtraData *extra)
{
	...
	if (path->jpath.path.parallel_workers > 0)
	{
		...
		path->jpath.path.rows =
			clamp_row_est(path->jpath.path.rows / parallel_divisor);
        //elog(INFO, "Fang Call path->jpath.path.rows %lf || %lf", path->jpath.path.rows, parallel_divisor);
	}
    ...
    //elog(INFO, "     ------>------>------> Fang final_cost_hashjoin %.0f and %.0f", inner_path_rows, outer_path_rows);

    /* Author: Fang WANG. Learning cardinality for re-optimization.
     * current_node_estimation is estimation card of hash join node
     * If we find a hash join has appeared in initial execution,
     * we would like to check if its child hash node is done work.
     * Once the child hash node is done work.
     * Case 1: the hash node is done work, but re-optimization can only use
     * built hash table.
     * Case 2: the hash node is done work, but we buffer the child results of
     * hash table. Hence, both the built hash table and the child results (of hash table)
     * is available.
     * */
    if(!lpce_check) {

        for(int i = 0; i < NODE_NUM_MAX; i++){
            //elog(INFO, "     ------>------>------> Fang node id %ld and  %ld  ", i, card_runLog[i].done_work);

            //the hash node has appeared in initial execution
            if(node_style[i] == 7
               && card_runLog[i].inner_relids == inner_path->parent->relids->words[0]
               && card_runLog[i].outer_relids == outer_path->parent->relids->words[0]){
                int hash_node_id = card_runLog[i].inner_node_id;

                //check if the hash node is done work
                if (card_runLog[hash_node_id].done_work){
                    //if its done work, set the hash node cost as 0
                    //inner_path->total_cost is the cost of hash node
                    startup_cost -= inner_path->total_cost;
                    inner_path->total_cost = 0;
                }
    ...
    //elog(INFO, "     ------> Fang final_cost_hashjoin: %lf  %lf", startup_cost, startup_cost + run_cost);
	path->jpath.path.startup_cost = startup_cost;
    path->jpath.path.total_cost = startup_cost + run_cost;
}
...
set_baserel_rows_estimate_standard(PlannerInfo *root, RelOptInfo *rel)
{
    ...
    elog(INFO, "Fang base relation size: %lf", nrows);
						   ...
						   List *restrictlist)
{
	if (set_joinrel_size_estimates_hook) {
        (*set_joinrel_size_estimates_hook)(root, rel,
                                           ...
                                           sjinfo,
                                           restrictlist);
        //elog(INFO, "    Fang call set_joinrel_size_estimates_hook");
    }
	else
						   ...
						   List *restrictlist_in)
{
	...
	switch (jointype)
	{
		case JOIN_INNER:
            ...
            elog(INFO, "Fang join size estimate: %lf = %lf %lf %lf %lf", nrows, outer_rows, inner_rows, fkselec, jselec);
			/* pselec not used */
			break;
		case JOIN_LEFT:
			nrows = outer_rows * inner_rows * fkselec * jselec;
    ...
    current_node_estimation = nrows;
    elog(INFO, "Fang join size: %lf", clamp_row_est(nrows));
