...
make_one_rel(PlannerInfo *root, List *joinlist)
{
    ...
    elog(INFO, "     ------>------> Fang make_one_rel make_one_rel");
	/*
...
standard_join_search(PlannerInfo *root, int levels_needed, List *initial_rels)
{
	...
	{
		ListCell   *lc;
        elog(INFO, "Fang all possible pairs of relations to be joined at level %d ", lev);
		/*
		...
		foreach(lc, root->join_rel_level[lev])
		{
			...
			if (lev < levels_needed)
                ...
                //elog(INFO, "Fang find and save cheapest paths for this rel of level %d ", lev);
