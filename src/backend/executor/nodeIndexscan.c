...
ExecEndIndexScan(IndexScanState *node)
{
    ...
    elog(INFO, "     ------>------> Fang ExecEndIndexScan");
	/*
...
ExecInitIndexScan(IndexScan *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> Fang ExecInitIndexScan at node %ld estimate tuples %.0f", indexstate->ss.ps.plan->plan_node_id, indexstate->ss.ps.plan->plan_rows);
	/*
    ...
    if(lpce_check){
        ...
        node_style[node_id]              = 2; //index only scan
