...
tts_minimal_store_tuple(TupleTableSlot *slot, MinimalTuple mtup, bool shouldFree)
{
    ...
    //elog(INFO, "     @@@@@@@@@@@ Fang AAAAAAAAAAAA %ld %ld %s", mtup->t_len, MINIMAL_TUPLE_OFFSET, mslot->minhdr.t_data);
//	if(test_purpose)
//        elog(INFO, "     ------> Fang AAAAAAAAAAAA %ld %ld %s", mtup->t_len, MINIMAL_TUPLE_OFFSET, (char *) mtup - MINIMAL_TUPLE_OFFSET);
//    if(test_purpose) {
//        elog(INFO, "     ------> Fang BBBBBBBBBBB %ld %ld ||   %s ", slot->tts_flags, mslot->minhdr.t_data->t_infomask2,
//             mslot->minhdr.t_data->t_choice.t_datum);
//    }
}
...
slot_getsomeattrs_int(TupleTableSlot *slot, int attnum)
{
	...
	if (unlikely(attnum > slot->tts_tupleDescriptor->natts)) {
        elog(ERROR, "Fang xxxxxxxxxxxxxxxxxxx %d", attnum);
