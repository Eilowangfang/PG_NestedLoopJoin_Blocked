...
#include "executor/nodeMaterial.h" //Fang
...
static MaterialState * ExecInitMaterialAtHashNode();
static void ExecMaterialAtHashNode(MaterialState *matstate, TupleTableSlot *slot);
...
MultiExecPrivateHash(HashState *node)
{
	...
	//Fang
	int tupleCount = 0;
    //elog(INFO, "     ------>------>------> Fang MultiExecPrivateHash enter once");
	...
	for (;;)
	{
		...
		tupleCount++;
        //Fang init a materialization
		//if(tupleCount == 1){
        //    node->matstate = ExecInitMaterialAtHashNode();
		//}

		//Fang
		//if(tupleCount % 100 == 100)
		//   print_slot(slot);
        //Fang feed tuple into materialization
        //ExecMaterialAtHashNode(node->matstate, slot);
 ...
 *		we want to collect the input from child node and materialize into a temp relation
 *		when the signal REOPTMATERIAL is set, we
 *		a set of co-operating backends.
 * ----------------------------------------------------------------
 */
MaterialState * ExecInitMaterialAtHashNode(){
    MaterialState *matstate;
    matstate = makeNode(MaterialState);
    matstate->tuplestorestate = NULL;
    matstate->eof_underlying = false;
    elog(INFO, "     ------>------>------> Fang ExecInitMaterialAtHashNode");
    return matstate;
}
 ...
 * ----------------------------------------------------------------
 */
void ExecMaterialAtHashNode(MaterialState *matstate, TupleTableSlot *slot){
    ...
    if (tuplestorestate == NULL)
    {
        ...
        matstate->tuplestorestate = tuplestorestate;
    }

    eof_tuplestore = true;
    if (eof_tuplestore && !matstate->eof_underlying)
    {
        if (TupIsNull(slot))
        {
            matstate->eof_underlying = true;
        }
...
void ExecEndMaterialAtHashNode(MaterialState *matstate){
    if (matstate->tuplestorestate != NULL)
        tuplestore_end(matstate->tuplestorestate);
    matstate->tuplestorestate = NULL;
    elog(INFO,"     ------> ------> Fang ExecEndMaterial");
}
...
ExecInitHash(Hash *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> Fang ExecInitHash at node %ld estimate tuples %.0f", hashstate->ps.plan->plan_node_id, hashstate->ps.plan->plan_rows);
    //elog(INFO, "     ------> Fang ExecInitHash###################inner %ld  outer %ld", hashstate->ps.plan->inner_relids, hashstate->ps.plan->outer_relids);
	...
	//Fang
    if(lpce_check){
        int node_id = hashstate->ps.plan->plan_node_id;

        card_runLog[node_id].estimate_card = hashstate->ps.plan->plan_rows;
        node_style[node_id]                = 5; //hash node
        card_runLog[node_id].outer_node_id = outerPlanState(hashstate)->plan->plan_node_id;
        card_runLog[node_id].outer_relids  = hashstate->ps.plan->outer_relids;
    }
...
ExecEndHash(HashState *node)
{
	PlanState  *outerPlan;
	//Fang
	/*when end the hash node, release materialization space*/
    //ExecEndMaterialAtHashNode(node->matstate);
	/*
	 * free exprcontext
	 */
	ExecFreeExprContext(&node->ps);
    elog(INFO, "     ------>------> Fang ExecEndSeqScan");
...
#endif
    elog(INFO, "     ------>------>------> Fang ExecHashJoinImpl -> ExecHashTableCreate");
	/*
					...
					uint32 hashvalue)
{
	...
	if (batchno == hashtable->curbatch)
	{
        ...
        //elog(INFO, "     ------>------>------> Fang ExecHashTableInsert %ld", tuple->t_len);
