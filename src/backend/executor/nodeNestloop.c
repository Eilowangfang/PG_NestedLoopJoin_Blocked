...
int outer_tuple_num = 0;
int inner_tuple_num = 1;
...
ExecNestLoop(PlanState *pstate)
{
	...
	for (;;)
	{
		...
		if (node->nl_NeedNewOuter)
		{
            ...
            //elog(INFO, "     ------> Fang ExecNestLoop outer tuple    %ld", outer_tuple_num);
            outer_tuple_num++;
			/*
			...
			if (TupIsNull(outerTupleSlot))
			{
                ...
                elog(INFO, "     ------> Fang ExecNestLoop Ending up");
				//Fang
...
static TupleTableSlot *
ExecNestLoop_SORT(PlanState *pstate)
{
    NestLoopState *node = castNode(NestLoopState, pstate);
    NestLoop   *nl;
    PlanState  *innerPlan;
    ...
    if (node->build_tuplestore && tuplestorestate == NULL)
    {
        ...
        elog(INFO, "     ------> Fang ExecNestLoop build up the tuplestore at node %ld ", node->js.ps.plan->plan_node_id);
    }
            ...
            * */
            if (TupIsNull(outerTupleSlot)) {
                ...
                if(lpce_check){
                    ...
                    //Fang
                    if(abs(outer_tuple_num - 0) < 2.0){
                        elog(INFO, "     ------> Fang ExecNestLoop TRIGGER RE-OPTIMIZATION @@@@@@@@@@@");
                        lpce_check    = false;
                        lpce_earlyend = true;
                        lpce_rerun    = true;
...
//            }
        ...
        elog(INFO, "     ------> Fang ExecNestLoop insert tuple into tuplestore at node %ld ", node->js.ps.plan->plan_node_id);
    }
...
ExecInitNestLoop(NestLoop *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> Fang ExecInitNestLoop at node %ld estimate tuples %.0f", nlstate->js.ps.plan->plan_node_id, nlstate->js.ps.plan->plan_rows);
    //elog(INFO, "     ------> Fang ExecInitNestLoop###################inner %ld  outer %ld", nlstate->js.ps.plan->inner_relids, nlstate->js.ps.plan->outer_relids);
	/*
...
ExecEndNestLoop(NestLoopState *node)
{
    ...
    elog(INFO, "     ------>------> Fang ExecEndNestLoop");
