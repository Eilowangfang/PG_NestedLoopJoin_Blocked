...
ExecSort(PlanState *pstate)
{
	...
	if (!node->sort_Done)
	{
        ...
        elog(INFO, "     ------> Fang SORT NODE id: %ld  card: %lf", node->ss.ps.plan->plan_node_id, outerNode_tuple_counter);
        ...
        card_runLog[outerNode_id].done_subplan = true;  //the child node belong to executeds subplan


        if(lpce_check && abs(outerNode_tuple_counter - 0) < 10.0){
            elog(INFO, "     ======> Fang SORT NODE id: %ld  TRIGGER REOPT", node->ss.ps.plan->plan_node_id);
            lpce_check    = false;
            lpce_earlyend = true;
            lpce_rerun    = true;
            ...
            int outerNode_id = outerPlanState(node)->plan->plan_node_id;
            card_runLog[outerNode_id].done_work = true;
        }
    }
    ...
    //Fang stop early
	if (lpce_earlyend){
	    //elog(INFO, "     ------> Fang ExecSort node id %ld terminate with time %lf", node->ss.ps.plan->plan_node_id, (double)(clock() - lpce_cuptime)/CLOCKS_PER_SEC);
        return NULL;
    }
	return slot;
}
...
ExecInitSort(Sort *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> Fang ExecInitSort at node %ld estimate tuples %.0f  %ld", sortstate->ss.ps.plan->plan_node_id, sortstate->ss.ps.plan->plan_rows,
    ...
    if(lpce_check){
        ...
        //elog(INFO, "     ------>------> Fang ExecInitSort outer node id %ld estimate tuples %.0f", card_runLog[node_id].outer_node_id, card_runLog[node_id].outer_card);
