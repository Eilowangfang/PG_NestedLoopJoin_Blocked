		...
		switch (node->hj_JoinState)
		{
			...
			case HJ_BUILD_HASHTABLE:
				...
				if (parallel)
				{
                    ...
                    elog(INFO, "     ------>------>------> Fang ExecHashJoinImpl Parallel build_barrier");
                ...
                /*
                 * Obtain the number of inner plan (hash node)
                 */
                if (innerPlanState(node)->instrument)
                        InstrEndLoop (innerPlanState(node)->instrument);

                elog(INFO, "     ------>------>------> Fang ExecHashJoinImpl HJ_BUILD_HASHTABLE DONE");
                //elog(INFO, "     ------>------> Fang ExecHashJoinImpl hash node end at total time: %.3f ", innerPlanState(node)->instrument->total);
                //elog(INFO, "     ------>------> Fang ExecHashJoinImpl hash node id %ld and outer node id %ld", innerPlanState(node)->plan->plan_node_id, outerPlanState(node)->plan->plan_node_id);
                //elog(INFO, "     ------>------> Fang ExecHashJoinImpl hash node estimate %.0f and outer node estimate %.0f", innerPlanState(node)->plan->plan_rows, outerPlanState(node)->plan->plan_rows);

                //Fang
                if (innerPlanState(node)->instrument){

                    elog(INFO, "     ------>------> Fang ExecHashJoinImpl hash node real %.0f and outer node real %.0f", innerPlanState(node)->instrument->ntuples, outerPlanState(node)->instrument->ntuples);
                    double hashNode_estimate_tuples = innerPlanState(node)->plan->plan_rows;
                    double hashNode_actual_tuples = innerPlanState(node)->instrument->ntuples;

                    //update the hash node info
                    //the hash node is completed work, and also its child node
                    //update the real card of hash node and its child node
                    int now_hashjoin_id = node->js.ps.plan->plan_node_id;
                    int hash_node_id = card_runLog[now_hashjoin_id].inner_node_id;
                    card_runLog[hash_node_id].node_card    = hashNode_actual_tuples;
                    card_runLog[hash_node_id].done_subplan = true;
                    int child_hash_node_id = card_runLog[hash_node_id].outer_node_id;
                    card_runLog[child_hash_node_id].node_card = hashNode_actual_tuples;
                    card_runLog[child_hash_node_id].done_subplan = true;


                    //end query
                    /* Author: Fang WANG. Learning cardinality for re-optimization.
                    * We check the real card of hash node (inner node at hash join node)
                    ...
                    * Once we detect the large error, we can set the child hash node as done work
                     *
                    * Case 1: the hash node is done work, but re-optimization can only use
                    * the built hash table.
                    * Case 2: the hash node is done work, but we have buffered the data of
                    * child node of hash table. Hence, both the built hash table and the intermediate
                    * child results (of hash table) is available.
                    * The intermediate child results actually is the data before structured into hash table.
                    *
                    * In case 2, the child node of hash node also needs being set as
                    * done work, so that the optimizer at re-optimization phase would realize the intermediate
                    * data is available to be utilized.
                    * */
                    if(lpce_check){
                        if(abs(hashNode_actual_tuples - 426103) <= 2.0){
                            lpce_check    = false;
                            lpce_earlyend = true;
                            lpce_rerun    = true;
                            elog(INFO, "     ------>------> Fang ExecHashJoinImpl done hash node: %d ", innerPlanState(node)->plan->plan_node_id);

                            //update info for hash node
                            //Case 1:
                            card_runLog[hash_node_id].node_card = hashNode_actual_tuples;
                            card_runLog[hash_node_id].done_work = true;
			...
			case HJ_NEED_NEW_OUTER:
                ...
                //elog(INFO, "     ------>------>------> Fang ExecHashJoinImpl HJ_NEED_NEW_OUTER");
				/* FALL THRU */
			...
			case HJ_NEED_NEW_BATCH:
				...
				else
				{
					if (!ExecHashJoinNewBatch(node)){
                        if(outerPlanState(node)->instrument){
                            /*
                            * Obtain the number of inner plan (hash node)
                            */
                            InstrEndLoop(outerPlanState(node)->instrument);
                            int outerNode_id = outerPlanState(node)->plan->plan_node_id;
                            double outerNode_actual_tuples   = outerPlanState(node)->instrument->ntuples;
                            card_runLog[outerNode_id].node_card = outerNode_actual_tuples;
                            card_runLog[outerNode_id].done_subplan = true;
                        }
...
ExecInitHashJoin(HashJoin *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> Fang ExecInitHashJoin at node %ld estimate tuples %.0f", hjstate->js.ps.plan->plan_node_id, hjstate->js.ps.plan->plan_rows);
    //elog(INFO, "     ------> Fang ExecInitHashJoin###################inner %ld  outer %ld", hjstate->js.ps.plan->inner_relids, hjstate->js.ps.plan->outer_relids);
    //elog(INFO, "     ------> Fang ExecInitHashJoin estate %s", estate->es_sourceText);
	...
	//elog(INFO, "     ------> Fang ExecInitHashJoin at inner node id %ld outer node id %ld", innerPlanState(hjstate)->plan->plan_node_id, outerPlanState(hjstate)->plan->plan_node_id);
    if(lpce_check){
        int node_id = hjstate->js.ps.plan->plan_node_id;

        card_runLog[node_id].estimate_card = hjstate->js.ps.plan->plan_rows;
        node_style[node_id]  = 7;

        card_runLog[node_id].inner_node_id = innerPlanState(hjstate)->plan->plan_node_id;
        card_runLog[node_id].outer_node_id = outerPlanState(hjstate)->plan->plan_node_id;

        card_runLog[node_id].inner_relids = hjstate->js.ps.plan->inner_relids;
        card_runLog[node_id].outer_relids = hjstate->js.ps.plan->outer_relids;
    }
...
{
    elog(INFO, "     ------>------> Fang ExecEndHashJoin ");
    //Plan	   *plan = innerPlanState(node)->plan;
    //Plan *plan = (Plan *)node;
    //elog(INFO, "     ------>------>------>  %.0f ", plan->plan_rows);
