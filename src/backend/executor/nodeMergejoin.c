	...
	for (;;)
	{
		...
		switch (node->mj_JoinState)
		{
			...
			case EXEC_MJ_INITIALIZE_OUTER:
                ...
                elog(INFO, "     ------> Fang ExecMergeJoin EXEC_MJ_INITIALIZE_OUTER");
				 ...
				 * EXEC_MJ_INITIALIZE_OUTER is actually first call of ExecSort().
				 * The work is to sort the data and store it in tuplesort.
				 * Merge node first call ExecSort() to sort data from outer path.
				 * Once we detect the large error and decide to early end the query,
				 * we first check the sort node and the child (only outer) of the
				 * sort node as done work.
				 * */
				if(lpce_earlyend){
                    int mergeJoin_node_id = node->js.ps.plan->plan_node_id;
                    //outer sort node of merge join node
                    int outerSortNode_id  = card_runLog[mergeJoin_node_id].outer_node_id;
                    card_runLog[outerSortNode_id].done_work = true;
                    //the child node of outer sort node
                    int child_outerSortNode_id = card_runLog[outerSortNode_id].outer_node_id;
                    card_runLog[child_outerSortNode_id].done_work = true;
				}
			...
			case EXEC_MJ_INITIALIZE_INNER:
                ...
                /* Author: Fang WANG. Learning cardinality for re-optimization.
                * EXEC_MJ_INITIALIZE_INNER is actually calling ExecSort().
                * The work is to sort the data and store it in tuplesort.
                * Merge node first call ExecSort() to sort data from inner path.
                * Once we detect the large error and decide to early end the query,
                * we first check the sort node and the child (only outer) of the
                * sort node as done work.
                * Note that here, we need to set the both outer and inner sort node of
                * merge join as done work
                * */
				if(lpce_earlyend){
                    elog(INFO, "     ------> Fang ExecMergeJoin UPPPDATE child status");
                    int mergeJoin_node_id = node->js.ps.plan->plan_node_id;
                    ...
                    //inner sort node of merge join node
                    int innerSortNode_id  = card_runLog[mergeJoin_node_id].inner_node_id;
                    card_runLog[innerSortNode_id].done_work = true;
                    //the child node of inner sort node
                    int child_innerSortNode_id = card_runLog[innerSortNode_id].outer_node_id;
                    card_runLog[child_innerSortNode_id].done_work = true;
                }
                elog(INFO, "     ------> Fang ExecMergeJoin EXEC_MJ_INITIALIZE_INNER");
...
ExecInitMergeJoin(MergeJoin *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> Fang ExecInitMergeJoin at node %ld estimate tuples %.0f", mergestate->js.ps.plan->plan_node_id, mergestate->js.ps.plan->plan_rows);
    //elog(INFO, "     ------> Fang ExecInitMergeJoin###################inner %ld  outer %ld", mergestate->js.ps.plan->inner_relids, mergestate->js.ps.plan->outer_relids);
	/*
    ...
    if(lpce_check){
        int node_id = mergestate->js.ps.plan->plan_node_id;
        card_runLog[node_id].estimate_card = mergestate->js.ps.plan->plan_rows;
        node_style[node_id]            = 8; //merge join
        ...
        card_runLog[node_id].inner_relids = mergestate->js.ps.plan->inner_relids;
        card_runLog[node_id].outer_relids = mergestate->js.ps.plan->outer_relids;

        //outer sort node
        card_runLog[outerPlan(node)->plan_node_id].outer_relids = mergestate->js.ps.plan->outer_relids;
        //inner sort node
        card_runLog[innerPlan(node)->plan_node_id].outer_relids = mergestate->js.ps.plan->inner_relids;
    }
...
ExecEndMergeJoin(MergeJoinState *node)
{
    ...
    elog(INFO, "     ------>------> Fang ExecEndMergeJoin ");
	/*
