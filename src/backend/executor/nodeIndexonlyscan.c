...
ExecEndIndexOnlyScan(IndexOnlyScanState *node)
{
    ...
    elog(INFO, "     ------>------> Fang ExecEndIndexOnlyScan");
...
ExecInitIndexOnlyScan(IndexOnlyScan *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> Fang ExecInitIndexOnlyScan at node %ld estimate tuples %.0f", indexstate->ss.ps.plan->plan_node_id, indexstate->ss.ps.plan->plan_rows);
    //elog(INFO, "     ------> Fang ExecInitIndexOnlyScan ExecInitIndexOnlyScan !!!!!!  %ld", indexstate->ss.ps.plan->outer_relids);
	/*
    ...
    if(lpce_check){
        int node_id = indexstate->ss.ps.plan->plan_node_id;
        card_runLog[node_id].estimate_card = indexstate->ss.ps.plan->plan_rows;
        node_style[node_id]               = 4; //index only scan
        card_runLog[node_id].outer_relids = indexstate->ss.ps.plan->outer_relids;
    }
