...
ExecMaterial(PlanState *pstate)
{
	...
	if (tuplestorestate == NULL && node->eflags != 0)
	{
        ...
        elog(INFO,"     ------> Fang ExecInitMaterial at node: eflags %ld", node->eflags);
...
//
//                test_purpose = true;
//                elog(INFO, "     ------> Fang ExecMaterial eof_tuplestore BBB %ld %ld %ld  %ld", eof_tuplestore, forward,
//                     tuplestore_tuple_count(tuplestorestate), natts);
//            }else
//                test_purpose = false;
		    return slot;
        }
	...
	if (eof_tuplestore && !node->eof_underlying)
	{
		...
		//elog(INFO, "     ------> Fang ExecMaterial CCC %ld %ld  %ld", eof_tuplestore, forward, temp_counter);
...
ExecInitMaterial(Material *node, EState *estate, int eflags)
{
    ...
    elog(INFO,"     ------> Fang ExecInitMaterial at node: %d", matstate->ss.ps.plan->plan_node_id);
    ...
    elog(INFO,"     ------> Fang ExecInitMaterial at node: eflags %ld", matstate->eflags);
    elog(INFO,"     ------> Fang ExecInitMaterial at node: EXEC_FLAG_REWIND %ld", EXEC_FLAG_REWIND);
    elog(INFO,"     ------> Fang ExecInitMaterial at node: EXEC_FLAG_BACKWARD %ld", EXEC_FLAG_BACKWARD);
    elog(INFO,"     ------> Fang ExecInitMaterial at node: EXEC_FLAG_MARK %ld", EXEC_FLAG_MARK);
	return matstate;
}
