...
//Fang
/* ----------------------------------------------------------------
 *				global variable used for re-optimization
 * ----------------------------------------------------------------
 */
#define NODE_NUM_MAX 20
extern bool lpce_check;
extern bool lpce_rerun;
extern bool lpce_earlyend;
extern bool lpce_reopt;

extern bool test_purpose;

/* node_style:
 * 0, 1(seq scan), 2(index scan), 3(tid scan), 4(index only scan)
 * 5(hash), 6(sort), 7(hash join), 8(merge sort join), 9(nested loop join)
 * */
extern int node_style[NODE_NUM_MAX];

extern struct card_Runlog{
    double node_card;     //real card
    double estimate_card; //node estimate card

    int    inner_node_id;
    int    outer_node_id;

    int    inner_relids; //all relations included in inner path of this node
    int    outer_relids; //all relations included in outer path of this node

    bool   done_subplan; //one node belongs to executed part
    bool   done_work;    //only the nodes we can make use of for re-optimization

}card_runLog[NODE_NUM_MAX];
